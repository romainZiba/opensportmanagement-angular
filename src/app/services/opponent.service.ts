...
export class OpponentService {
  ...
  getOpponents(teamId: number) {
    this.http
      .get<Opponent[]>(`/teams/${teamId}/opponents`, { withCredentials: true })
  ...
  createOpponent(teamId: number, opponent: Opponent): Promise<boolean> {
    return new Promise(resolve => {
      const subscription = this.http
        .post<Opponent>(`/teams/${teamId}/opponents`, opponent, {
          withCredentials: true
        })
        .subscribe(
          createdOpponent => {
            this.opponentsSource.next(
              this.opponentsSource.getValue().push(createdOpponent)
            );
            resolve(true);
          },
          error => resolve(false)
