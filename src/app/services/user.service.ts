...
export class UserService {
  ...
  updateUser(
    ...
    return new Promise(resolve => {
      const subscription = this.http
        .put<User>(`accounts/me`, user, { withCredentials: true })
        .subscribe(updatedUser => {
          if (updatedUser !== null) {
            this.userSource.next(updatedUser);
            this.teamService
              .updateTeamMember(teamId, licenceNumber)
  ...
  confirmAccount(confirmationId: string, password: string): Promise<boolean> {
    ...
    return new Promise(resolve => {
      const subscription = this.http
        .put<User>(`accounts/confirmation`, confirmationBody)
        .subscribe(
          updatedUser => (updatedUser === null ? resolve(false) : resolve(true))
        );
      setTimeout(function() {
        subscription.unsubscribe();
      }, 5000);
