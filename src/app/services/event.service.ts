...
export class EventService {
  ...
  getEvent(id: number) {
    this.http
      .get<Event>(`/events/${id}`, { withCredentials: true })
  ...
  updateEvent(id: number, event: EventCreateUpdate): Promise<boolean> {
    return new Promise(resolve =>
      this.http
        .put<Event>(`/events/${id}`, event, { withCredentials: true })
        .subscribe(
          updatedEvent => {
  ...
  getMessages(eventId: string) {
    return this.http.get<AppMessage[]>(`/events/${eventId}/messages`, {
  ...
  postMessage(eventId: string, body: string) {
    ...
    message.body = body;
    const subscription = this.http
      .post<AppMessage>(`/events/${eventId}/messages`, message, {
        withCredentials: true
      })
      .subscribe();
    setTimeout(function() {
