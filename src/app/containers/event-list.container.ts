...
export class EventListSmartComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  private _loadEvents(currentPage: number, pageSize: number, retrieveAll: boolean) {
    const sub = this.selectedTeam$
      .map(team => this.eventService.getEvents(team._id,  currentPage, pageSize, retrieveAll))
      .subscribe();
