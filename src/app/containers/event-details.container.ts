...
export class EventDetailsSmartComponent extends BaseComponent
  ...
  participate(isParticipating: boolean) {
    this.eventService
      .participate(this.eventId, isParticipating)
      .then(success => {
        if (!success) {
  ...
  sendReminder() {
    this.eventService.sendReminder(this.eventId).then(success => {
      success
        ? this.openSnackBar("Notifications envoyés avec succès")
        : this.openSnackBar("Une erreur s'est produite");
    });
  }

  openRegistration() {
    this.eventService.openRegistration(this.eventId).then(success => {
      success
        ? this.openSnackBar("Notifications envoyés")
        : this.openSnackBar("Une erreur s'est produite");
    });
  }
