...
export class TeamCreationComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  createTeam(form: TeamForm) {
    this.teamService.createTeam(form.name, form.sport).then(
      success => success ? (this.openSnackBar('Equipe créé avec succès'), this.location.back())
