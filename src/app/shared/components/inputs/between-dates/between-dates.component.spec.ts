...
describe("BetweenDatesComponent", () => {
  ...
  describe("change from date", () => {
    it("should be invalid when from date is before min date", () => {
      ...
      expect(tester.fromDateMinDateError).toEqual({ "actual": "2018-01-01", "minDate": "2018-11-01" });
    });
    ...
    it("should be invalid when toDate is before fromDate", () => {
      ...
      expect(tester.isValid).toBeFalsy();
      expect(tester.toDateMinDateError).toEqual({ "actual": "2018-11-01", "minDate": "2018-12-01" });
    });
  });
});
